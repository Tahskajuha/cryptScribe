...
- **<u>Zero-Plaintext Backend</u>:**
  The backend is a stateless API server that **never receives or stores plaintext data**. Encryption/decryption is handled solely on the client-side.
...
- **<u>Skins</u>:**
  ...
  > For details on the IPC layer, CLI usage and how to implement custom skins, see [IPCDOCS.md](IPCDOCS.md).
...
- **<u>Opt-In Password Reset</u>:**
  The app allows the user to choose whether they want the option to reset their password later. For maximum privacy, the user can enter unique gibberish instead of a valid email address which will effectively disable the password reset feature.
...
- [ ] Security fixes and rate-limiting.
...
- **Auto-Unset Marks:** Pressing _Backspace_ on an empty block automatically removes active marks.
...
- Input rules aim for a familiar Markdown-like workflow while supporting advanced formatting like subscript, superscript and inline highlights.
- Custom extensions like _EscapeChar_ allow literal insertion of characters that would normally trigger formatting.
...
- Copy the .env.example file in the repository to create the .env file.
